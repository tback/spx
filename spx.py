...
def main():
    ...
    logging_group.add_argument('-l', '--loglevel', choices=['DEBUG','INFO', 'WARNING', 'ERROR', 'CRITICAL'],
    ...
    off_parser.set_defaults(func=lambda args: spx.off(args.host, args.username, args.password))

    off_parser = subparsers.add_parser('on', parents=[common_parser])
    off_parser.set_defaults(func=lambda args: spx.on(args.host, args.username, args.password))
    ...
    monitor_parser.set_defaults(func=lambda args: spx.monitor(args.host, args.interval, args.username, args.password))

    args = parser.parse_args()

    logging.basicConfig(level=getattr(logging, args.loglevel), stream=sys.stderr)
